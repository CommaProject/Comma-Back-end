...
public class UserControllerTest {
	...
	public void loginUser() throws Exception {
		...
		final TokenDTO token = TokenDTO.builder().code(1).accessToken("accessToken").build();
		doReturn(token).when(userService).login(any(RequestUserDTO.class), any(HttpServletResponse.class));
		...
		final TokenDTO response = gson.fromJson(
				resultActions.andReturn().getResponse().getContentAsString(StandardCharsets.UTF_8), TokenDTO.class);
	...
	public void notExistUser() throws Exception {
		...
		doThrow(exception).when(userService).login(any(RequestUserDTO.class), any(HttpServletResponse.class));
		...
		// then
		resultActions.andExpect(status().isOk());
		final MessageDTO response = gson.fromJson(
